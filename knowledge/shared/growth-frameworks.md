...
**Sources consulted**:
- alirezarezvani/claude-skills (https://github.com/alirezarezvani/claude-skills; growth marketing reference)
...
**Source licence**: MIT for alirezarezvani/claude-skills; other named sources are per-source-terms and cited, not reproduced.
...
## AI-Era PLG: Agent-Driven Loops + Time-to-Value (2026)

**Adapted from**: ProductLed "The Evolution of Product-Led Growth: PLG x AI" (productled.com, 2026); aakashg.com "Product-Led Growth 2026: 7-Layer Guide" (news.aakashg.com, 2026)
**Source licence**: no-license-but-publicly-cited
**V2V refinements**:
- Integrated the agent-driven turn into the existing four-loop archetype taxonomy rather than treating it as a separate model
- Promoted time-to-value from an activation sub-metric to a first-class loop input under AI conditions
- Cross-referenced the OS-core `agentic-commerce.md` pack for the machine-customer / selling-to-an-agent surface rather than restating it
- Preserved the V2V caveat that an agent-as-actor loop is a variant of the existing loops, not a replacement for the sustainable-loop discipline

The four loop archetypes earlier in this pack share a hidden assumption: a **human** takes the core action that produces the loop's output. In 2026 that assumption no longer always holds, and the change is load-bearing for how growth loops are designed.

### The Shift: User-Driven → Agent-Driven

Public framing (ProductLed) describes three PLG eras: **1.0** (the user does all the work — "Builders"), **2.0** (AI assists the user — "Editors"), and **3.0** (AI does much of the work while the user reviews — "Reviewers"). The practical consequence for growth is that the **work between intention and outcome is collapsing**, and the time-to-value gap that used to be an activation sub-metric becomes the dominant growth lever. When a product can deliver its core value in seconds rather than a multi-step human setup, activation friction (the highest-leverage stage in most growth models) compresses, and the loop's cycle time shortens.

This is not a new loop archetype. It is a **modifier on the existing four**: each loop can now have an agent-as-actor variant where part of the core action is performed by AI (the product's own agents) or by the **user's** AI agent acting on their behalf.

| Existing loop | Human-driven (the pack's baseline) | Agent-driven variant (2026) |
|---|---|---|
| Acquisition / Viral | User creates value, shares with network | Product agent generates a shareable artifact automatically; user's agent surfaces/recommends the product to other agents |
| Engagement / Habit | User takes core action, gets value, returns | Product agent proactively delivers value (the return is initiated by the system, not the human) — watch for the engagement-without-intent failure mode |
| Content / SEO | User generates content, content indexed | Agent-generated content at scale — quality and authenticity become the binding constraint, not volume |
| Paid | Revenue funds acquisition | Largely unchanged; unit economics discipline still governs |

### Time-to-Value as a First-Class Loop Input

In the AI era, instrument time-to-value (TTV) as a primary loop input alongside the existing inputs (invite rate, activation rate, K-factor). The hypothesis to test: **halving median TTV moves activation and loop cycle time more than any single funnel-step optimization**. Use the pack's existing experiment framework (ICE scoring, hypothesis template, guardrails) to validate this per product — TTV gains can hide a quality regression, so retention stays the guardrail. The sustainable-growth principle is unchanged: an agent-driven loop that compounds is a loop; an AI feature that produces a one-time spike is not.

### Machine-Customer Surface (Cross-Reference, Not Restated)

When the actor entering the loop is a **machine customer** — an AI shopping/buying agent acquiring or transacting on a human's behalf — the acquisition and conversion surface changes materially (agent-legibility, protocol-as-channel, agentic checkout). That motion is owned by the OS-core **`agentic-commerce.md`** pack (authored in the prior 2026-Q2 wave). Growth-loop design that targets machine customers should consume that pack for the selling-to-an-agent mechanics; this section only flags that the agent-as-actor loop variant and the machine-customer motion are the same theme seen from two lenses (growth-loop design here, GTM/transaction motion there).

> **Operating note**: The agent-driven turn amplifies the pack's existing anti-patterns rather than retiring them. "Growth hacks without a model" and "scaling before learning" are more dangerous, not less, when an AI feature can manufacture a vanity-metric spike overnight. Build the compounding loop; let AI shorten its cycle time.
