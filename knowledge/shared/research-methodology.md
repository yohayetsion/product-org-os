...
<!-- Section added 2026-06-24 (PBAW Full-Workforce Refresh, Marketing wave). Additive — process, CRAAP, and synthesis frameworks above unchanged. -->
**Adapted from**:
  - Verian Group, "Synthetic Sample in Social Research: significant limitations of AI generated responses" (2025-12, veriangroup.com)
  - V2V cross-pack: `discovery-methods.md` §11.1 "AI-Moderated Interviewing and the Cadence Ceiling" (PBAW OS-core wave, 2026-06-24)
**Source licence**: cited-not-claimed (Verian is public reporting; discovery-methods §11.1 is an internal V2V pack)
**V2V refinements**: cross-reference the AI-moderated-interviewing treatment instead of re-deriving it; extend the existing CRAAP test (Source Evaluation Framework above) rather than replacing it.

**When to use**: AI is now in two parts of the research workflow — *generating* data (synthetic respondents, AI-moderated interviews) and *contaminating* desk-research sources (AI-generated articles, hallucinated citations). This section adds the guardrails; it does not change the Research Process Framework above.

**AI-moderated interviewing — cross-reference, do not re-derive.** AI-moderated qualitative interviewing (which breaks the classic one-customer-per-week cadence ceiling) is treated in full in **`discovery-methods.md` §11.1 "AI-Moderated Interviewing and the Cadence Ceiling."** The V2V caveat from that section applies here verbatim: AI moderation scales *evaluative/confirmatory* interview volume, **not** the generative "what is the real problem?" leap or the shared-context a research trio builds. When AI-moderated interviews feed this pack's synthesis steps, label them as evaluative-volume input and keep generative discovery human-led. See §11.1 for the cadence mechanics; this pack does not restate them.

**Synthetic sample (AI-generated respondents) — directional, not a real-respondent replacement.** "Synthetic sample" / "digital twins" — survey answers generated by an LLM instead of by people — are now offered as a faster, cheaper insight path. Documented limitations are significant (Verian, 2025-12): synthetic responses regress toward training-data averages, miss genuine novelty and edge segments, and can fabricate plausible-but-wrong sentiment. The V2V posture:
- Use synthetic sample for **pre-testing** (survey-instrument debugging, hypothesis triage, question-wording checks), **never** as the evidence base for a recommendation.
- A finding that will appear in a recommendation must be backed by **real respondents** — synthetic sample does not satisfy the Triangulation rule above as an independent source.
- Always **label synthetic-derived findings as synthetic** in the synthesis template, in the "What We Don't Know (Limitations)" section.

**CRAAP extension — detecting AI-generated / hallucinated sources (additive to the Source Evaluation Framework above).** The classic CRAAP test (Currency, Relevance, Authority, Accuracy, Purpose) predates AI-generated content flooding desk research. Add these checks before trusting a source:
- **Provenance** — Is there a named, verifiable human author and a real publisher? AI-generated content farms produce confident, well-formatted, authorless pages.
- **Citation reality** — Do cited sources, studies, and quotes actually exist? Hallucinated citations look real; spot-check at least the load-bearing ones against the original.
- **Specificity sniff** — Vague, hedge-heavy, source-free assertions ("studies show," "experts agree") with no traceable origin are an AI-generation tell.
- **Circular-citation check** — AI-generated pages frequently cite each other. Trace a claim to a primary source, not to another summary.

> AI changes how data is *produced*, not the standard of evidence. Synthetic and AI-moderated inputs are triage and pre-test; real respondents and verifiable primary sources remain the bar for anything that drives a decision.
