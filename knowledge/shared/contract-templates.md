...
- oneNDA (https://onenda.org/) — standardized NDA framework

**Source licence**: Apache-2.0 for open-agreements; oneNDA licence unverified, template source cited only.
...
## AI-as-Feature & AI Procurement Clauses (2026)

> ⚠️ **Not legal advice.** This output is a drafting and triage aid generated by a product-organization skill, not counsel. No attorney-client relationship is created by its production or use. Jurisdiction-specific questions, contested matters, and any decision with material legal or regulatory consequences require review by a licensed attorney in the relevant jurisdiction. Do not rely on this output as the sole basis for any legal, compliance, or contracting decision.
>
> **Jurisdiction Assumed:** U.S. federal (with Delaware as default state-law fallback for commercial terms). If your jurisdiction differs, treat every clause and finding below as a hypothesis to verify with local counsel. Public-sector procurement assumes the U.S. FAR/GSAR regime; commercial-vendor terms assume U.S. UCC Article 2 / common-law contract principles.

**Adapted from**:
- GSA proposed AI procurement clause GSAR 552.239-7001 (draft released 2026-03-06; U.S. General Services Administration — public domain)
- Cited analyses: Holland & Knight, "GSA's Proposed AI Clause: A Deep Dive" (2026-03-18); Baker Botts, "GSA's New AI Clause" (2026-03-25); FAS, "How State Governments Should Purchase AI" (2026-06); JD Supra, "When Your SaaS Vendor Becomes an AI Company Overnight" (2026-06)

**Source licence**: GSAR draft and FAR/GSAR text are U.S. public domain; cited law-firm analyses referenced as commentary, not reproduced.

**V2V refinements**:
- Reframed the GSAR 552.239-7001 procurement instrument and AI-as-feature commentary into a reusable AI-rider clause checklist (5 clause families) for product-org contract drafting
- Added per-section sensitive-skill scaffolding (disclaimer + Jurisdiction Assumed + Findings + Reviewer Checklist + Cannot Assess Without) as specified directly in this section
- Wired cross-references to the AI BOM (`ai-bom.md`) as the inventory that audit/transparency clauses populate, and to EU AI Act deployer documentation (`ai-act-readiness.md`)
- Added the P0 training-data-use carve-out → Limitation-of-Liability cap-carve-out linkage so the carve-out is not silently subject to the 12-month-fee cap

**Date-stamped**: 2026-06-24 · **VERIFY**: GSAR 552.239-7001 was in DRAFT/comment status as of the 2026-03-06 release. Confirm current adoption status, final clause number, and effective date against acquisition.gov / SAM.gov before relying on it in any live procurement.

### Why this section exists

Two distinct 2026 developments make AI-specific contract terms a baseline drafting concern rather than a bespoke add-on:

1. **AI-as-feature in existing vendor relationships.** SaaS and platform vendors are switching on AI features inside contracts that were signed before AI was contemplated — often routing customer data through new sub-processors and foundation-model providers without a fresh negotiation. The risk surface (training-data use, output IP, new sub-processors, model substitution) is created by an amendment or a silent feature toggle, not a new agreement. Existing MSAs frequently lack the clauses that would govern it.
2. **Public-sector AI procurement clauses.** GSA's draft GSAR 552.239-7001 (released 2026-03-06) signals a federal-procurement move toward mandatory AI transparency, disclosure, and data-rights terms for vendors on GSA Schedules. Whether or not a given organization sells to government, the clause is a leading indicator of the disclosure terms commercial counterparties will begin to demand.

### The AI Rider Checklist (five clause families)

When AI is, or may become, a material component of a vendor relationship, the agreement (or an AI rider/addendum to an existing MSA) should address each of the following. These are **drafting prompts for counsel, not finalized clause language** — binding text requires attorney drafting and review.

| # | Clause family | What it should govern | Risk if absent |
|---|---------------|----------------------|----------------|
| 1 | **Model-substitution notice** | Advance written notice before the vendor changes the underlying foundation/fine-tuned model, model version, or model provider powering the service; right to evaluate or object before the change takes effect | Silent model swaps change accuracy, bias, latency, data-residency, and sub-processor posture with no contractual visibility |
| 2 | **Training-data-use carve-out** | Explicit prohibition (or tightly scoped, opt-in-only permission) on the vendor using customer data, inputs, prompts, or outputs to train, fine-tune, or improve any model; survival post-termination. **Carve this obligation OUT of the general Limitation-of-Liability cap** (see linkage note below) — a 12-month-fee cap is inadequate for irreversible absorption of customer data into model weights | Customer data absorbed into model weights is effectively unrecoverable; creates IP, privacy, and confidentiality exposure |
| 3 | **Output-IP allocation** | Who owns AI-generated output; warranties (or express disclaimers) on output originality and non-infringement; allocation of third-party-IP-infringement risk in generated output | Ambiguous output ownership and uncapped infringement risk default against the party that can least prove provenance |
| 4 | **Sub-processor AI disclosure** | Disclosure and approval rights over AI/foundation-model sub-processors (Anthropic, OpenAI, Google, Bedrock, Azure OpenAI, etc.); flow-down of data-protection and training-use restrictions to those sub-processors; alignment with the customer's AI BOM | Undisclosed AI sub-processors break the sub-processor chain assumed in the DPA and the customer's AI inventory |
| 5 | **Audit / transparency rights** | Right to obtain the vendor's AI documentation (model card, AI BOM, evaluation results, intended-use and limitation statements); audit rights proportionate to risk tier; cooperation in the customer's own regulatory documentation. **Where the procured system may be high-risk under the EU AI Act, scope this clause to the deployer's Article 11 / Annex IV documentation needs — but note the high-risk timeline is in flux (EU AI Act "Digital Omnibus" provisional agreement 2026-05-07, not yet formally adopted as of 2026-06-06; verify before relying)** | Customer cannot satisfy its own EU AI Act Article 11 / Annex IV documentation or FCRA/state-AI-disclosure obligations without upstream transparency |

**Limitation-of-Liability linkage (P0).** The clause-2 training-data-use carve-out connects directly to the **Limitation of Liability** "Carve-Outs from Cap" row (see *Key Negotiation Clauses → Limitation of Liability*) and the **Red Flags** checklist (*Contract Review Checklist → Red Flags*). Treat unauthorized training-data use the same way the cap table treats IP infringement, confidentiality breach, and data breach: a carve-out from the general liability cap (with a separate super-cap or uncapped treatment as negotiated). Leaving the P0 carve-out subject to the standard 12-24 months'-fees cap understates an irreversible harm — flag it at Red-Flag severity if the counterparty insists on capping it.

### Public-sector instrument: GSAR 552.239-7001 (draft, 2026-03-06)

For procurements on a GSA Schedule (and as a forward indicator for commercial deals), the draft GSAR 552.239-7001 clause moves toward requiring vendors to disclose AI use, provide transparency documentation, and accept government data-rights and oversight terms. Treat it as a **leading-indicator checklist**, not settled law:

- Disclosure of whether and how AI is used in the offered product/service
- Transparency documentation obligations (intended use, limitations, performance)
- Data-rights and data-use restrictions favoring the government customer
- Oversight, audit, and notice obligations on material AI changes

> **VERIFY — INLINE CAVEAT (do not strip when lifting this clause):** GSAR 552.239-7001 was in **draft/comment status** at its 2026-03-06 release. It is **NOT a binding FAR/GSAR provision** until adopted. Confirm final clause number, adoption status, and effective date at acquisition.gov before citing it in any live procurement or representing it to a counterparty as a binding requirement. A reader inserting this clause into an agreement must carry this caveat with it.

## Findings

1. **AI-as-feature gap in legacy MSAs.** *What:* Pre-AI MSAs typically lack the five AI rider clause families. *Why it matters:* A vendor can introduce AI processing of customer data by amendment or feature toggle with no governing terms. *Severity:* P1. *Suggested next step:* Counsel to assess whether an AI rider should be appended to material existing vendor agreements at next renewal.
2. **Model-substitution blind spot.** *What:* Absence of model-change notice (clause 1). *Why it matters:* Silent model swaps alter data-residency, sub-processor, bias, and performance posture. *Severity:* P1. *Suggested next step:* Counsel to draft notice + objection-window language scaled to risk tier.
3. **Training-data-use ambiguity left under the liability cap.** *What:* No explicit carve-out (clause 2), or carve-out left subject to the general 12-month-fee cap. *Why it matters:* Customer data absorbed into model weights is unrecoverable; IP/privacy/confidentiality exposure that a fee-based cap cannot remedy. *Severity:* P0. *Suggested next step:* Counsel to draft a prohibition or tightly-scoped opt-in with post-termination survival AND a carve-out from the liability cap.
4. **GSAR clause status uncertainty.** *What:* GSAR 552.239-7001 cited as draft. *Why it matters:* Relying on draft procurement language as binding misstates the obligation. *Severity:* P1. *Suggested next step:* Verify adoption status at acquisition.gov before use in any live procurement.

## Reviewer Checklist
- [ ] Jurisdiction confirmed (U.S. federal / Delaware fallback; public-sector vs commercial regime correctly selected)
- [ ] Material facts verified against the actual vendor agreement and any AI amendment/feature notice
- [ ] GSAR 552.239-7001 adoption status, final clause number, and effective date verified at acquisition.gov
- [ ] AI rider clause families 1-5 reviewed for scope, caps, carve-outs, and survival by licensed counsel
- [ ] P0 training-data-use carve-out confirmed carved OUT of the general liability cap
- [ ] Cross-referenced against the customer's DPA, sub-processor list, and AI BOM (`ai-bom.md`)
- [ ] EU AI Act deployer documentation needs assessed where applicable, with Digital Omnibus timeline status verified (`ai-act-readiness.md`)
- [ ] All P0 findings addressed or explicitly accepted as risk

## Cannot Assess Without Licensed Counsel
- Enforceability and drafting of binding model-substitution, training-data-use, output-IP, sub-processor, and audit clauses in the operative jurisdiction
- Whether GSAR 552.239-7001 (draft 2026-03-06) is in force, its final text, and its applicability to a specific procurement vehicle
- Output-IP ownership and third-party-infringement risk allocation under applicable copyright and contract law
- Whether the procured AI system is high-risk under EU AI Act Article 6 + Annex III, and the deployer documentation obligations that follow (defer to `ai-act-readiness.md` + counsel; note the Digital Omnibus high-risk timeline is in flux and travels with any high-risk-dependent clause)
- FCRA / state-AI-disclosure obligations where the AI system produces employment, credit, or other consequential-decision outputs (apply the structural guardrails above and cross-reference `ai-bom.md`)
- Negotiation leverage, cap structures, and indemnity allocation specific to the counterparty and deal
