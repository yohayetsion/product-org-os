...
> See `agent-runtime-patterns.md` for framework selection + agent memory architecture.
...
## 2026-06 Currency Addendum (added 2026-06-12, dev-team radar Stream B)

Two sections of this pack have drifted from production reality. The corrections below supersede the older text where they conflict; depth lives in the two dev-team packs cited at the end.

### Correction 1 — Guardrails: tool outputs are the front-line injection surface

The guardrail sections above (Safety Guardrail Template; Production LLM Patterns → Guardrails) frame injection defense around **user input** and lead with **pattern matching** ("ignore previous instructions" regexes). Both framings are stale for agent systems as of 2026:

- **The dominant production injection surface is content the agent ingests mid-run** — above all **tool outputs**, plus retrieved/fetched content and even third-party tool *descriptions* (the MCP vector). Per the OWASP Top 10 for Agentic Applications 2026 (genai.owasp.org, as of 2026-06) and OWASP's Q1-2026 exploit round-up, injected instructions in agent-processed content (emails, PDFs, RAG documents, web content, deceptive tool outputs) drive most agentic production security failures. A guardrail stack that only screens user input misses this entirely.
- **Pattern matching is demonstrably insufficient** for indirect injection. Current OWASP-class guidance (LLM Prompt Injection Prevention Cheat Sheet, cheatsheetseries.owasp.org): treat ALL text that influences agent reasoning as untrusted and screen it with a **trained classifier**, keeping regex only as a cheap pre-filter for known exfiltration patterns (suspicious URLs, markdown-image exfiltration, secret/PII shapes) on the output side.

Engineering shape: there are **four sanitization seams** — user input → model; retrieved/fetched content → model; **tool output → model context**; model output → display/action — and each gets explicit middleware, not prompt-level self-policing.

### Correction 2 — Structured outputs: strict schema mode is universal

The "Structured Output Reliability" approaches list above is stale on two facts (as of early 2026):

- **All three major providers — OpenAI, Anthropic, and Google — support native strict-schema structured outputs** (Anthropic added native support late 2025). The "(where supported)" hedge on Anthropic no longer applies. Strict mode uses constrained decoding (schema compiled to a token mask), giving effectively guaranteed *schema* compliance vs. a published 2–5% mismatch rate for JSON-mode-without-schema.
- Constrained-decoding is therefore no longer a local-model-only technique (approach 2 above) — it is the hosted-API default via strict mode.

Unchanged caveat, now with its own 2026 literature: **schema-valid ≠ semantically correct**. Boundary validation (Zod/Pydantic with semantic refinements) plus a single bounded correction retry, ending in a **typed failure** rather than a degraded guess, remains required.

### Where the depth lives

For implementation guidance, apply the guardrail and observability patterns in this public pack directly:

The observability tooling list above (Langsmith/Helicone era) should be read through that second pack: the portable layer is OTel GenAI semantic conventions; sinks (Langfuse v4 and peers) are swappable behind it.
