...
## 2026 API Spec Deltas (added 2026-06-24, PBAW Wave 1 — `chief-architect`)

**Adapted from**:
  - OpenAPI Specification v3.2 — OpenAPI Initiative (openapis.org/blog/2025/09/23/announcing-openapi-v3-2; learn.openapis.org/upgrading/v3.1-to-v3.2.html)
  - AsyncAPI v3.0 / v3.1 — AsyncAPI Initiative (asyncapi.com/blog/release-notes-3.0.0; asyncapi.com/blog/release-notes-3.1.0)
**Source licence**: OpenAPI Specification (Apache-2.0); AsyncAPI Specification (Apache-2.0)
**V2V refinements**: framed as a delta layer over the REST/GraphQL/gRPC/event-driven guidance above; mapped each change to a design decision the pack already covers (streaming, auth, versioning, docs).

> **Verify before relying on specifics:** spec field names and release dates below were accurate to the cited sources as of 2026-06-24. Re-check the OpenAPI/AsyncAPI release notes before depending on a specific field.

This section is **additive** — the REST/GraphQL/gRPC/event-driven decision frameworks above remain authoritative and unchanged. It records the spec-level movement that post-dates this pack's v1.0.0 baseline.

### OpenAPI 3.2 (released September 2025)

OpenAPI 3.2 is the first release to treat several modern patterns as first-class. The changes that affect V2V API-design guidance:

- **First-class streaming.** Streaming responses (Server-Sent Events, JSON Lines, sequential multipart feeds) are now described directly via `itemSchema` and `prefixEncoding`, instead of being modeled as one-off hacks. **Design implication:** if you expose a streaming endpoint (common for AI/LLM features — cross-ref `ai-ml-patterns.md`), describe it in the spec rather than documenting it out-of-band.
- **OAuth 2.0 Device Authorization Flow.** New supported flow for end-user authorization on input-constrained devices (TVs, kiosks, CLI). **Design implication:** if a consumer surface lacks a browser, this is now the spec-blessed auth path — add it to the auth-requirements decision rather than inventing a custom handoff.
- **Hierarchical (structured) tags.** Tags can nest, grouping operations under categories / subdomains / business units instead of a flat list. **Design implication:** large APIs get navigable docs without external tooling.
- **QUERY method.** OpenAPI now recognizes the `QUERY` HTTP method alongside GET/POST/PUT — chiefly for read-only search endpoints that need a request body without the cache/idempotency confusion of POST. **Design implication:** a cleaner answer to "complex search via GET (body-less) vs POST (non-idempotent-looking)" than the workarounds the pack's REST section otherwise implies.
- **Multipart clarity.** Clearer definitions for mixing file uploads with structured JSON metadata in one `multipart/form-data` request.

### AsyncAPI 3.0 / 3.1 (event-driven APIs)

For the event-driven side of the pack:

- **Channel / Operation decoupling.** A Channel now defines the messages available at an address; an Operation references a Channel and selects a subset, carrying an `action` property. **Design implication:** channels become reusable across documents — model the channel once, reference it from multiple operations.
- **`send` / `receive` replaces `publish` / `subscribe`.** The old pub/sub verbs (a frequent source of "from whose perspective?" confusion) are replaced by explicit `send` / `receive` on the Operation's `action`. **Design implication:** event API definitions read unambiguously; update any AsyncAPI 2.x definitions on migration.
- **3.1.0** is a non-breaking minor over 3.0.0 — safe to adopt without a migration project.

### Cross-cutting 2026 trend (verify)
Multiple 2026 sources note **AI-powered API gateways** (auto-generated docs, adaptive rate limiting, anomaly detection) as an emerging operational layer. Treat as a directional signal, not a design mandate — **verify** maturity before architecting a dependency on it.
