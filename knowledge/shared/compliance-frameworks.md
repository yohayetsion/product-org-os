...
## 0. Strategic Posture — Compliance as Consequence

Compliance is the *consequence* of well-run decisioning, not the goal of the system. A product organization that runs its decisions through Charters, records assumptions, and attests at the Reviewer Checklist gate naturally produces the artifacts compliance regimes require — without organizing the work around the regime. See `product-org-plugin/PRINCIPLES.md` Move C ("Compliance-as-Secondary") for the strategic statement.

This pack catalogues the frameworks (SOC2, ISO 27001, GDPR, EU AI Act, NIST CAISI, CSA AICM, FCRA, and others) so that the decisioning system's outputs can be mapped to the obligations each framework expresses. The pack is a translation layer between upstream decisioning artifacts and downstream regulator-recognized vocabulary — not a workplan for organizing the product org around compliance.
...
1. **Pointer references only.** Every conceptual claim is accompanied by a pointer to the regulator source in the embedded canonical source URLs. No content is lifted from source documents.
...
3. **Date-sensitive content lives in the sidecar.** Effective dates, transition deadlines, version numbers, and "status as of" claims go to the embedded current-status authority, never the main pack.
...
Each framework below is cited by its stable structural elements (article numbers, control categories, risk tiers, obligation buckets). For effective dates, version numbers, and status updates, see the embedded current-status authority. For canonical URLs, see the embedded canonical source URLs.
...
- **EDPB guidance**: The EDPB issues guidelines on recurring interpretive questions (automated decision-making, AI, legitimate interests, international transfers). These are the closest thing to a regulator position the pack cites. See the embedded canonical source URLs for the EDPB guidance index.
...
- **Automated Decision-Making Technology (ADMT) rules**: CPRA authorizes rules on automated decision-making and profiling. Draft rules have been proposed by the CPPA; final rules are not yet in effect as of the pack's last update (see the embedded current-status authority for current status).
...
**Published by**: European Parliament and Council. Adopted 2024. Phased entry into application across 2024-2027 (see the embedded current-status authority for the schedule).
...
**Pointer discipline**: The EU AI Act text is available at EUR-Lex (see the embedded canonical source URLs). This pack does NOT reproduce article text; it cites article numbers and obligation bucket names. For current application dates see the embedded current-status authority. For interpretive questions, defer to licensed EU counsel — the pack explicitly does NOT take interpretive positions on AI Act application.
...
**Pointer discipline**: The IMDA Agentic AI Governance Framework text is available at the IMDA website (see the embedded canonical source URLs). This pack cites the 7 risk categories and the structural relationship to the Model AI Governance Framework without reproducing framework text. For current version and status see the embedded current-status authority.
...
- **Why the pack carries it**: sector-specific banking directives are a worked case of the custom-framework path in `/compliance-audit` - a regime with no off-the-shelf adapter, defined inline by the user. It is cited here because downstream skills may reference banking-sector obligations as a structural input.

**Pointer discipline**: PPL text is available via the PPA; Directive 361 is published by the Bank of Israel. Both are in Hebrew with unofficial English translations circulating; the pack does not cite unofficial translations as authoritative. See the embedded canonical source URLs for URLs (canonical paths flagged for verification).
...
The pack and its sidecars are re-verified quarterly. Next verification due: **2026-07-11**. The `last_verified` header in the embedded current-status authority is updated at each verification cycle. The main pack file's `Last updated` field is updated only when structural content changes; date-sensitive content lives in the sidecar.
...
- **Effective date approaches** — An obligation becomes applicable (e.g., EU AI Act high-risk system obligations applying 2026-08-02). Verify that the embedded current-status authority reflects the status change.

### 7.3 Re-verification procedure

For each triggered re-verification:
1. Pull the canonical source URLs from the embedded canonical source URLs
2. Compare against the embedded current-status authority entries for date-sensitive content
3. Compare against this pack's structural element claims
4. Update the embedded current-status authority with new timestamped entries (never delete old entries; append with dates)
5. If structural elements have changed, update the main pack and increment minor version
6. Log the verification in a dated entry at the bottom of the embedded current-status authority
...
Canonical URLs for all sources cited above are maintained in the embedded canonical source URLs. The main pack intentionally does not list URLs inline; mechanical re-verification pulls from the embedded canonical source URLs, which is the single source of truth for source URLs.
...
**Authoring discipline**: First-principles authoring from public regulator and standards-body sources. No law firm citations. No vendor or consultancy content. Every conceptual claim is pointer-backed to the embedded canonical source URLs.
...
**Sidecar files**:
- `the embedded current-status authority below` — date-sensitive content (first instance of the sidecar pattern per `.claude/rules/delegate-first.md` shared skill orchestrator)
- `the embedded canonical source URLs below` — canonical URL manifest for mechanical re-verification
...
*Version 1.1.0 — published 2026-04-11. Previous version: 1.0 (2026-02-14). Change summary: added Singapore Agentic AI Governance Framework (pointer reference), added EU AI Act as explicit pointer reference with article-level structural elements, added ISO/IEC 42001, added Israeli PPL + Directive 361, added control-to-obligation mapping table (Section 3), added framework selection rubric (Section 4), added overlap matrix (Section 5), added boundary statements with `/compliance-audit`, `/ai-regulatory-audit`, and `/ai-control-audit` (Section 6), added re-verification cadence (Section 7), introduced embedded current-status authority and embedded canonical source URLs sidecars, removed vendor and consultancy references.*

## Embedded current-status authority

# Compliance Frameworks — Current Status (embedded section)

**last_verified**: 2026-04-11
**next_verification_due**: 2026-07-11 (quarterly)
**re_verification_triggers**: regulator announcement, case law, enforcement action, framework revision, obligation effective date approaching
**parent_pack**: `../compliance-frameworks.md`

---

## Purpose

This embedded status section carries all date-sensitive claims for the Compliance Frameworks knowledge pack: effective dates, transition deadlines, version numbers, regulator guidance cadence, and "status as of" claims. The main pack cites structural elements (article numbers, control categories, risk tiers) that do not change. This file carries everything that does.

**Why split**: The main pack is intended to be stable version-to-version. If a deadline slips or a version number changes, the main pack should not need to be touched — only this embedded status section. This separation makes quarterly re-verification cheap and reduces risk that stale date claims contaminate downstream skill outputs.

---

## Date-sensitive entries

### EU AI Act (Regulation (EU) 2024/1689)

- **Adopted**: 2024-07-12 (European Parliament); formal publication in the Official Journal followed
- **Published in Official Journal**: 2024-07-12
- **Enters into force**: 2024-08-01 (20 days after OJ publication)
- **Prohibition provisions apply**: 2025-02-02 (6 months after entry into force)
- **GPAI model obligations apply**: 2025-08-02 (12 months after entry into force)
- **High-risk system obligations apply (Annex III)**: 2026-08-02 (24 months after entry into force)
- **High-risk obligations for AI embedded in regulated products (Annex I)**: 2027-08-02 (36 months after entry into force)
- **Full application**: 2027-08-02
- **GPAI Code of Practice status as of 2026-04-11**: [CHECK EU AI OFFICE for current status and final text availability]
- **AI Office status**: Established within the European Commission (DG CNECT). Operational.
- **Verified source**: EUR-Lex — see the embedded canonical source URLs below
- **Verification note**: The phased application timeline is defined in the Regulation itself (final provisions). Individual implementing acts, harmonized standards, and codes of practice are published on an ongoing basis and should be checked at the EU AI Office page.

### Singapore Agentic AI Governance Framework

- **Published by**: IMDA (Infocomm Media Development Authority)
- **Relationship to Model AI Governance Framework**: complements the existing Model AI Governance Framework (original 2019, updated 2020)
- **Status as of 2026-04-11**: [VERIFY current version and publication date at IMDA]
- **Voluntary nature**: non-binding guidance for Singapore-deployed AI systems
- **Verified source**: IMDA AI governance page — see the embedded canonical source URLs below
- **Verification note**: IMDA frameworks are typically revised on a multi-year cycle. Confirm latest revision date before downstream skills cite specific language.

### Singapore Model AI Governance Framework

- **First published**: 2019
- **Second edition**: 2020
- **Status as of 2026-04-11**: Current; complemented by the Agentic AI Governance Framework for agent-specific risks
- **Verified source**: IMDA — see the embedded canonical source URLs below

### NIST AI Risk Management Framework

- **Version**: 1.0
- **Published**: January 2023
- **Generative AI Profile**: Released July 2024
- **Status as of 2026-04-11**: Stable; check NIST for any updates
- **Verified source**: NIST AI RMF page — see the embedded canonical source URLs below

### ISO/IEC 42001 (AI Management Systems)

- **First edition published**: December 2023
- **Status as of 2026-04-11**: Stable standard; check ISO for amendments or corrigenda
- **Paywall**: Standard document is behind ISO paywall (not free to read)
- **Verified source**: ISO — see the embedded canonical source URLs below

### ISO/IEC 27001

- **Current edition**: 2022 (published October 2022)
- **Transition from 2013 edition**: Organizations certified under the 2013 edition had a transition period to 2025; as of 2026-04-11 the 2022 edition is the sole active version
- **Annex A structure change**: 93 controls in 4 themes (2022) vs 114 controls in 14 domains (2013)
- **Status as of 2026-04-11**: Stable
- **Verified source**: ISO — see the embedded canonical source URLs below
...
- **Status as of 2026-04-11**: Stable; no revision in progress. EDPB publishes interpretive guidelines on an ongoing basis.
- **EDPB guidance cadence**: EDPB publishes new and updated guidelines multiple times per year. Check the EDPB guidance index before citing specific interpretive positions.
- **Verified source**: EUR-Lex for Regulation text; EDPB for guidelines — see the embedded canonical source URLs below

### California CCPA / CPRA

- **CCPA effective**: 2020-01-01
- **CPRA amendments effective**: 2023-01-01
- **CPRA enforcement**: 2023-07-01 (delayed by court decision from original 2023-01-01 date)
- **Automated Decision-Making Technology (ADMT) rules**:
  - **Status as of 2026-04-11**: Draft rules have been proposed by the California Privacy Protection Agency (CPPA); final rules are not yet in effect
  - **Expected finalization**: [CHECK CPPA rulemaking page for current status]
  - **Verify at**: CPPA rulemaking page — see the embedded canonical source URLs below

### HIPAA

- **Privacy Rule, Security Rule, Breach Notification Rule**: Stable federal regulations
- **HHS OCR guidance on AI in healthcare**: HHS has issued periodic guidance on AI and healthcare. Check HHS OCR for current positions before citing.
- **Status as of 2026-04-11**: Stable; no major revision in progress
- **Verified source**: HHS HIPAA pages — see the embedded canonical source URLs below

### Israeli Protection of Privacy Law (PPL) 5741-1981

- **Original law**: 1981
- **2025 amendment**: Expands PPL with GDPR-aligned provisions (breach notification, DPO designation, extraterritorial scope). Phased effective dates.
- **Status as of 2026-04-11**: [VERIFY current effective dates of 2025 amendment provisions with the Privacy Protection Authority]
- **Verified source**: Israel Privacy Protection Authority — see the embedded canonical source URLs below

### Bank of Israel Directive 361

- **Scope**: Israeli banking sector information security and technology risk management
- **2024 amendments**: Added AI-specific risk management provisions
- **Status as of 2026-04-11**: In force; [VERIFY latest amendment status at Bank of Israel]
- **Verified source**: Bank of Israel Supervisor of Banks — see the embedded canonical source URLs below

### SOC 2 (AICPA Trust Services Criteria)

- **Current TSC reference**: 2017 Trust Services Criteria (with 2022 points of focus update)
- **Status as of 2026-04-11**: Stable
- **AICPA guidance on AI in SOC 2**: AICPA has issued practice guidance on AI controls within SOC 2; check AICPA for current practice aids.

### OECD AI Principles

- **Adopted**: 2019
- **Updated**: 2024 (update to reflect generative AI considerations)
- **Status as of 2026-04-11**: Current (2024 update)
- **Verified source**: OECD.AI — see the embedded canonical source URLs below

---

## Regulator guidance cadence reference

Use this table to anticipate when quarterly re-verification is likely to surface changes:

| Regulator / Body | Guidance Cadence | Primary Channel |
|---|---|---|
| EU AI Office | Ongoing — implementing acts, codes of practice | AI Office page, Commission website |
| EDPB (European Data Protection Board) | Multiple per year | EDPB guidance index |
| IMDA (Singapore) | Multi-year cycle for framework revisions | IMDA AI governance page |
| NIST | Multi-year for framework, ongoing for profiles | NIST AI RMF page |
| ISO/IEC | 3-5 year cycle for standard revisions | ISO catalog |
| California CPPA | Ongoing rulemaking | CPPA rulemaking page |
| HHS OCR | Ongoing guidance | HHS HIPAA page |
| Bank of Israel | Periodic directive amendments | Supervisor of Banks page |
| AICPA | Periodic TSC and practice aid updates | AICPA assurance services page |

---

## Verification log

### 2026-04-11 — Initial embedded section creation

- Embedded section created as part of Compliance Frameworks Knowledge Pack v1.1.0
- All entries above are as-of 2026-04-11 based on public regulator sources
- Entries marked `[CHECK ...]` or `[VERIFY ...]` are flagged for specific verification before downstream skills cite them
- Next verification due: 2026-07-11

---

## When downstream skills cite this embedded status section

Downstream skills (`/compliance-audit`, `/ai-regulatory-audit`, `/ai-control-audit`) MUST:
1. Check `last_verified` date before citing any date-sensitive claim
2. If `last_verified` is older than 90 days and the skill is producing regulator-facing output, flag the staleness in the skill's output
3. Never cite entries marked `[CHECK ...]` or `[VERIFY ...]` as authoritative without fresh verification
4. Cite by pointing to this embedded status section + the relevant canonical source URL in the embedded canonical source URLs below, not by copying the date into the skill's output without attribution

## Embedded canonical source URLs

# Compliance Frameworks — Canonical Sources (embedded section)


**last_verified**: 2026-04-11
**parent_pack**: this public pack
**companion_embedded section**: the embedded current-status authority

---

## Purpose

This file is the canonical URL manifest for all sources cited in the Compliance Frameworks knowledge pack. The main pack cites frameworks by name; this file carries the URLs for mechanical re-verification. Quarterly re-verification pulls each URL, checks for version or "last updated" changes, and reflects findings in the embedded current-status authority.

**Scope discipline**:
- Regulator primary sources and recognized standards bodies only
- No law firm publications
- No vendor or consultancy content
- No think-tank non-public sources
- No secondary interpretive blog posts
- Academic governance programs allowed where they produce public policy research, not advisory content
...
- **EU AI Act (Regulation (EU) 2024/1689)** — EUR-Lex canonical text
  - https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=OJ:L_202401689
  - Verification: check for consolidated versions and corrigenda at EUR-Lex

- **GDPR (Regulation (EU) 2016/679)** — EUR-Lex canonical text
  - https://eur-lex.europa.eu/eli/reg/2016/679/oj
  - Verification: check for consolidated versions at EUR-Lex

- **EDPB (European Data Protection Board) — guidelines, recommendations, best practices index**
  - https://www.edpb.europa.eu/our-work-tools/general-guidance_en
  - Verification: check the "latest publications" feed; EDPB publishes new guidelines multiple times per year

- **EU AI Office**
  - https://digital-strategy.ec.europa.eu/en/policies/ai-office
  - Verification: check for implementing acts, codes of practice status, and GPAI Code of Practice final text

### United States — Federal

- **NIST AI Risk Management Framework**
  - https://www.nist.gov/itl/ai-risk-management-framework
  - Verification: check for AI RMF version updates and new profiles (Generative AI Profile was added 2024-07)

- **NIST Cybersecurity Framework 2.0**
  - https://www.nist.gov/cyberframework
  - Verification: check for CSF version updates
...
- **HIPAA (US Department of Health and Human Services)**
  ...
  - Verification: check HHS OCR for updated guidance on AI in healthcare

### United States — California

- **CCPA / CPRA (California Attorney General)**
  - https://oag.ca.gov/privacy/ccpa
  - Verification: check CA OAG for enforcement priorities and interpretive guidance

- **California Privacy Protection Agency (CPPA)**
  - https://cppa.ca.gov
  - Rulemaking page: https://cppa.ca.gov/regulations/
  - Verification: check CPPA rulemaking for ADMT (Automated Decision-Making Technology) rules status

### Singapore

- **IMDA AI Governance (Model AI Governance Framework + Agentic AI Governance Framework)**
  - https://www.imda.gov.sg/how-we-can-help/ai-governance
  - Verification: confirm current versions of both frameworks; [verify specific path to Agentic AI Governance Framework document]

- **Personal Data Protection Commission (PDPC) Singapore**
  - https://www.pdpc.gov.sg
  - Verification: check for PDPA advisory guidelines touching AI

### Israel

- **Israel Privacy Protection Authority (PPA)**
  - https://www.gov.il/en/departments/the_privacy_protection_authority
  - Verification: confirm canonical URL; check for 2025 PPL amendment effective dates

- **Bank of Israel — Supervisor of Banks (Directive 361)**
  - https://www.boi.org.il/en/banking/bankingsupervision/
  - Verification: [verify specific path to Directive 361 and its amendments]
...
- **Information Commissioner's Office (ICO) — AI guidance**
  ...
  - Verification: UK GDPR is structurally similar to EU GDPR but diverging; check ICO for current UK-specific positions
...
- **UK Department for Science, Innovation and Technology (DSIT)**
  ...
  - Verification: UK takes an outcomes-based AI regulatory approach with sector regulators; check DSIT for the current AI regulatory framework publication
...
- **ISO/IEC 42001 (AI management systems)**
  ...
  - Paywall: standard document purchase required
  - Verification: check ISO catalog for amendments and related standards (42005, 42006)

- **ISO/IEC 27001:2022 (Information security management systems)**
  - https://www.iso.org/standard/27001
  - Paywall: standard document purchase required
  - Verification: check ISO catalog for 27001 amendments

- **ISO/IEC 27701 (Privacy information management)**
  - https://www.iso.org/standard/71670.html
  - Paywall: standard document purchase required
  - Verification: relevant for privacy extension to ISO 27001

- **AICPA Trust Services Criteria (SOC 2)**
  - https://www.aicpa-cima.com/resources/download/2017-trust-services-criteria-with-revised-points-of-focus-2022
  - Verification: check AICPA for practice aid updates touching AI controls in SOC 2
...
- **OECD AI Principles**
  ...
  - Full OECD.AI Policy Observatory: https://oecd.ai
  - Verification: check for updates to the principles and the Policy Observatory dashboards

- **Council of Europe Framework Convention on AI**
  - https://www.coe.int/en/web/artificial-intelligence
  - Verification: check for state signatures and ratifications

---

## Public governance research programs (academic / non-profit)

These are allowed as secondary references for governance research, not for regulatory interpretation:

- **Partnership on AI**
  - https://partnershiponai.org
  - Scope: multi-stakeholder research on responsible AI

- **GovAI / Centre for the Governance of AI**
  - https://www.governance.ai
  - Scope: academic AI governance research

- **Stanford HAI — AI Index**
  - https://hai.stanford.edu/research/ai-index-report
  - Scope: annual state-of-AI report including policy and regulation trends

---

## Mechanical re-verification instructions

For quarterly re-verification (and triggered re-verifications), the Compliance Officer or a delegated agent runs through the following procedure for each URL above:

1. **Fetch the page** — attempt the URL. If it returns a 404 or redirect to an unrelated page, flag in the embedded current-status authority and search for the replacement canonical URL.
2. **Check "last updated" or version number** — compare against the corresponding entry in the embedded current-status authority.
3. **Scan for new publications** — for index pages (EDPB guidance index, CPPA rulemaking page, EU AI Office page), scan for new guidelines, rules, or statements published since the last verification.
4. **Update the embedded current-status authority** — add a timestamped entry under "Verification log" at the bottom of the file. If a version has changed, add a new dated entry under the relevant framework section rather than overwriting old content.
5. **If structural elements change** — update the main pack (this public pack) and increment minor version (1.1.x → 1.2.0 for structural changes; 1.1.1 for documentation fixes without scope change).
6. **If a URL is paywalled** — note the paywall in this file; do not attempt to reproduce content behind the paywall. The standards body's catalog page is the canonical reference even when the standard document itself is paywalled.
...
- **Law firm publications, alerts, and memos** — excluded by pack policy
- **Compliance tooling vendor content** — excluded by pack policy
- **Consultancy blog posts and white papers** — excluded by pack policy
- **Trade association advisory guidance** (unless issued by a regulator) — excluded by pack policy
- **Secondary commentary** — excluded; only primary regulator and standards-body sources

If a downstream skill needs interpretive guidance on how a regulator has applied a framework in a specific case, that is a question for licensed counsel, not this knowledge pack. The pack's scope is structural reference, not interpretation.

---

## Source verification log

### 2026-04-11 — Initial source manifest

- Created as part of Compliance Frameworks Knowledge Pack v1.1.0
- All URLs above are recorded as canonical source pointers as of 2026-04-11
- URLs marked `[verify specific path]` require confirmation of the exact document location before downstream skills rely on them
- Next verification due: 2026-07-11
