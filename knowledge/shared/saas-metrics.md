...
**Source licence**: MIT for alirezarezvani/claude-skills; T2D3 is cited under per-source terms and no source text is reproduced.
...
| NRR | >80% | >100% | >110% | >115% |
| GRR | >70% | >80% | >85% | >90% |
| Gross Margin _(pre-AI baseline — see §2026: AI-COGS Gross-Margin Compression)_ | >60% | >65% | >70% | >75% |
...
**Caveat**: These are median benchmarks for VC-backed B2B SaaS. Bootstrapped, PLG, vertical SaaS, and usage-based models may have different healthy ranges. The **Gross Margin** row above is a *pre-AI* baseline; for any company shipping meaningful AI features, read it together with the §2026: AI-COGS Gross-Margin Compression section below.

---

## 2026: AI-COGS Gross-Margin Compression

**Added 2026-06-24.**

**Adapted from**:
  - "The AI COGS Problem: SaaS Gross Margin Compression 2026" (saasmag.com/ai-cogs-saas-gross-margin-compression, dated 2026-05-15) — public-company gross-margin data + the recovery playbook
  - ICONIQ Growth, 2026 State of AI Bi-Annual Snapshot (iconiq.com/growth, Jan 2026) — AI product gross-margin + inference-to-revenue figures
  - Bessemer Venture Partners, State of AI 2025 + AI Pricing & Monetization Playbook (bvp.com/atlas) — LLM-native margin band + outcome-based pricing
  - Ben Murray / The SaaS CFO — Inference Efficiency Ratio (thesaascfo.com)

**Source licence**: per-source-terms (public industry documentation; figures are cited, not asserted by V2V)

**V2V refinements**: the date-stamp / re-verify discipline below + tying the Rule-of-40 asterisk to this pack's existing Rule of 40 and SaaS Operating Benchmarks sections.

The core 2026 development the metrics above do not yet reflect: **AI inference is a real, variable cost of goods, and it compresses the ~80% software gross margin that this pack's benchmarks assume.** Cited figures (verify before reuse — see caveat): ICONIQ's Jan-2026 snapshot reported average AI-product gross margin at **52%** (up from 41% in 2024, 45% in 2025) with inference consuming **~23% of revenue** at scaling-stage AI B2B companies; Bessemer placed LLM-native gross margins around **65%**; for traditional SaaS bolting AI onto existing seats, the per-seat hit cited is **~12-17 points** of gross margin. None of these are V2V numbers — they are dated third-party data points, reproduced for orientation, not as benchmarks.

### Inference Efficiency Ratio (new line item)

```
Inference Efficiency Ratio = AI-related Revenue / AI-related Inference Cost
```

| Ratio | Interpretation |
|-------|---------------|
| >5 | Healthy — inference cost well covered |
| 3-5 | Watch — compounds with usage growth |
| <3 | Structural problem — worsens as usage scales |

Track inference as its own COGS line, not buried in generic hosting. The ratio is highly sensitive to product-design choices made long before the cost appears (per-keystroke frontier-model calls vs. batching + caching + small-model routing), so it is a **product-spec metric, not only a FinOps metric**.

### Rule of 40 — gross-margin asterisk

The Rule of 40 (see Efficiency / Rule of 40 sections above) is misleading across different gross-margin regimes. A 25%-growth / 80%-GM business scores materially better than a 25%-growth / 67%-GM business at identical FCF margin. **Benchmark within gross-margin bands** — never compare a 2026 AI SaaS at ~67% GM against a 2022 cloud SaaS at ~82% GM on a raw Rule-of-40 number. The same applies to the SaaS Operating Benchmarks table above: its Gross Margin row is a pre-AI baseline.

> **V2V caveat — these are emerging bands, not settled norms** [Owner: 🧮/🌟]: the 60-70% "new normal" AI-SaaS gross-margin band and the 52% average are **2026 in-flight figures**, improving quarter over quarter, drawn from a small set of disclosures and one analyst snapshot. **Date-stamp every figure, cite the source, and re-verify at each refresh.** Do NOT present any single AI-era gross-margin number as a canonical V2V benchmark, and do NOT let a lower AI-era GM read as "worse business" in isolation — a 65%-GM AI SaaS growing 60% with strong NRR can be a better business than a 78%-GM SaaS growing 18%. The framework needs nuance, not nostalgia.
