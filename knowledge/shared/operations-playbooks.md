...
**Example fragment (from ExampleCo MRR reconciliation SOP):**
```
trigger:
  type: schedule
  condition: "First business day of each calendar month, 08:00 UTC"
...
- **Source**: where the input is pulled from — `shopify-cli orders get`, `stripe-cli events list`, `artifact-store://finance/statements/`, `manual: operator provides`
...
**Example fragment (from ExampleShop fulfillment SOP):**
```
inputs:
  - name: order
    type: shopify.order
  ...
  - name: shipping_config
    type: config.json
    source: "ExampleShop/operations/shipping-config.json"
    required: true
```
...
**Example fragment (from ExampleCo expense approval SOP):**
```
decision_rights:
  steps:
...
- **artifact_destination**: where the tangible outputs land — e.g., `artifact-store://finance/recon/YYYY-MM/`, `s3://ExampleShop-ops/fulfillment/{order_id}/`
...
evidence_trail:
  log_schema:
    - run_id
    - sop_name: "ExampleShop-fulfillment"
    - sop_version: "1.0.0"
    - order_id
    - started_at
    - completed_at
  ...
  artifact_destination: "artifact-store://ExampleShop/operations/fulfillment/YYYY/MM/{order_id}/"
  retention: "per the applicable record-retention policy"
...
**Example fragment (from ExampleCo MRR reconciliation SOP, foot-and-tie):**
```
verification_test:
...
```yaml
sop_name: [kebab-case identifier]          # e.g., ExampleShop-fulfillment
...
inputs:
  ...
  - name: shipping_config
    type: config.json
    source: "ExampleShop/operations/shipping-config.json (carrier rules per market)"
    required: true
...
decision_rights:
  steps:
    ...
    - step_id: handle-stockout
      ...
      escalation: "ceo (accountable owner)"
      escalation_trigger: "decision unresolved > 12 business hours"
      acceptance_signal: "operations-dir posts decision in #ExampleShop-ops with order_id"
...
exception_handling:
  ...
  - exception: carrier_api_unreachable
    ...
    branch: "persist checkpoint, switch to manual label generation queue, notify operations-dir in #ExampleShop-ops"
...
verification_test:
  name: "ExampleShop-fulfillment-closeout-check"
  criterion: |
  ...
  frequency: "every run"
  failure_action: "mark run status=FAILED; open incident in #ExampleShop-ops; block next run for same order_id until resolved"
```

**Audit note.** A process engineer auditing this SOP has all 7 fields populated concretely, with named roles, specific carriers, locales, and log schemas. Every step has a completion signal. Every exception has a recovery path. The only `[TBD]` values are operational numbers that live in ExampleShop's config, not in this reference.

---

### Worked Example 2: ExampleCo Monthly MRR Reconciliation SOP

**Context (dated: 2026-04-11).** The company is on Stripe (test mode transitioning to production) and runs enterprise contracts alongside self-serve subscriptions. Monthly MRR reconciliation ties together three independent ledgers — Stripe recognized revenue, internal ledger MRR entries, and bank deposits from Stripe payouts. This SOP is the canonical C2.5 example because its verification test IS a foot-and-tie check. Values marked `[per deal]` and `[TBD]` are placeholders for deal-specific and org-specific numbers.

```yaml
sop_name: ExampleCo-mrr-reconciliation-monthly
version: 1.0.0
...
trigger:
  type: schedule
  condition: "First business day of each calendar month, 08:00 UTC"
  detector: "cron job `ExampleCo-mrr-recon-monthly` on finance-runner instance"
...
decision_rights:
  steps:
    ...
    - step_id: apply-deal-specific-adjustment
      decider: "fpa-analyst"
      criterion: "Example Bank deal uses [per deal] milestone-based billing; SOP applies milestone schedule from contract_register"
      escalation: "cfo"
...
steps:
  ...
  - step_id: apply-contract-adjustments
    action: "Apply deal-specific adjustments per contract_register (e.g., Example Bank milestone billing)"
    owner: "fpa-analyst"
...
evidence_trail:
  log_schema:
    ...
    - variance_pct
    - step_events[]: "{step_id, outcome, completed_at, duration_ms}"
  artifact_destination: "artifact-store://ExampleCo/finance/reconciliation/YYYY/{YYYY-MM}/"
  retention: "per the applicable corporate record-retention policy"
  access: "read: cfo, fpa-analyst, ceo; write: fpa-analyst only"

verification_test:
  name: "ExampleCo-mrr-foot-and-tie"
  criterion: |
...
Financial control SOPs (per `fpa-analyst`'s 7-dimension rubric in C2.5) MUST populate `verification_test` with a foot-and-tie check — a three-way (or more) internal consistency test across independent sources. The ExampleCo MRR reconciliation example above is the canonical pattern. When an FP&A analyst applies the C2.5 rubric to an SOP, the first dimension checked is **Internal Consistency**, and the evidence for that dimension lives in the `verification_test` field of this schema. If the verification test is not a foot-and-tie (or equivalent independent-source reconciliation), the SOP fails the C2.5 rubric regardless of how polished the rest of the SOP is.

Non-financial SOPs (like ExampleShop fulfillment above) are not held to the foot-and-tie bar — their verification tests can be carrier status checks, API response validation, state-transition assertions — but they still need a named, runnable test. The C2.5 rubric only governs the financial subset; the 7-field schema governs all SOPs.
