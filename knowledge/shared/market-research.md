...
**Sources consulted**:
- Anthropic knowledge-work-plugins (https://github.com/anthropics/knowledge-work-plugins; marketing/competitive-brief)
...
### AI Synthetic Sample — Use With Caveats (2026)

**Sources consulted**:
  - MIT Sloan Management Review, "Gain Consumer Insight With Generative AI" (2026-04, sloanreview.mit.edu) — digital-twin/LLM-respondent framing
  - Qualtrics X4 2026 coverage, "Insight Is Cheap: Qualtrics Is Selling Something Harder" (2026-03, cmswire.com) — synthetic panels as a shipped product
  - Verian Group, "Synthetic Sample in Social Research: significant limitations of AI generated responses" (2025-12, veriangroup.com) — the validity caveat
**Source licence**: per-source-terms for the named public reporting; figures are cited, not reproduced.
**Our determination**: describes-public-method.
**V2V refinements**: lead with the validity caveat before the capability; position synthetic sample as a *sampling-methodology* choice that augments — never replaces — real respondents; gate recommendations on real data.

**When to use this section**: A vendor or stakeholder proposes "synthetic sample," "digital twins," or "synthetic respondent panels" — LLM-generated survey answers standing in for real people — to get insight faster and cheaper. Decide here whether and how to use it as a *sampling* choice. (For the broader AI-in-the-workflow guardrails — AI-moderated interviewing, AI-source contamination in desk research — see `research-methodology.md` §"AI in the Research Workflow"; this section does not restate the workflow angle.)

**Read the validity caveat first.** Synthetic sample has *significant, documented limitations* (Verian, 2025-12): LLM-generated responses regress toward training-data averages, miss genuine novelty and low-frequency edge segments, and can produce plausible-but-fabricated sentiment that reads like real data. A synthetic panel will confidently answer questions a real population would split or refuse on. Confidence in the output is not evidence of validity.

**What is real in 2026 (cited, directional):** LLM "digital twins" of consumer segments are positioned to make research faster and cheaper (MIT SMR, 2026-04), and synthetic panels now ship as commercial product features (Qualtrics X4, 2026-03). The capability exists; the open question is methodological fitness for a given decision, not whether the tool runs.

**The V2V posture — augmentation and pre-test, not a respondent replacement:**

| Defensible use (augment / pre-test) | Not defensible (replacement) |
|-------------------------------------|------------------------------|
| Debugging the survey instrument (wording, flow, skip logic) before fielding to real people | Sizing a market or preference on synthetic responses |
| Hypothesis triage — narrowing which questions are worth asking real respondents | Reporting synthetic sentiment as the voice of the customer |
| Rough directional pre-read to shape a real study | Any finding that drives a go/no-go, pricing, or roadmap decision |
| Generating candidate questions/segments for human review | Standing in for hard-to-reach or low-frequency real segments |

**Operating rules** (consistent with this pack's evidence standards):
1. **Real respondents are the bar for recommendations.** A finding that will appear in a recommendation must be supported by real-respondent data. Synthetic sample does not count as an independent source for triangulation.
2. **Label synthetic-derived findings as synthetic** everywhere they appear — in the synthesis, in the deck, in the limitations section. Never let a synthetic read be mistaken for fielded data.
3. **Use it to spend real-respondent budget better, not to avoid spending it.** The legitimate ROI is a sharper instrument and tighter hypotheses going into the real study — not skipping the real study.
4. **Disclose to stakeholders.** If synthetic sample informed any part of the work, say so up front; the cost of an undisclosed synthetic finding driving a decision is far higher than the research it saved.

> Synthetic sample makes insight *cheaper to draft*, not *safer to decide on*. Treat digital twins as a pre-test and triage layer; gate every decision on real respondents.
