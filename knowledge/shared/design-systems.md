...
- **W3C Design Tokens Community Group (DTCG) format** — adoption growing, spec nearing 1.0 as of April 2026. Style Dictionary 4.x supports it natively. Figma Variables export to DTCG is available. This is the format to commit to. *(Superseded — see 2026-06-24 update below: the spec is no longer "nearing 1.0"; it reached its first stable version 2025.10 on 2025-10-28.)*
...
#### DTCG reached first stable version — 2025.10 (update as of 2026-06-24)

**Adapted from**: W3C Design Tokens Community Group, "Design Tokens specification reaches first stable version" (2025-10-28) + Design Tokens Format Module 2025.10 (designtokens.org/tr/2025.10/) — verified live 2026-06-24.
**Source licence**: W3C Community Final Specification Agreement (FSA) for the Design Tokens Format Module 2025.10; specification cited, not reproduced.
**V2V refinements**: re-anchored the "commit to DTCG" recommendation from a moving-target hedge to a stable-spec commitment; mapped the stable capabilities to this pack's multi-channel token-exposure and AI-agent-readiness preconditions; no fabricated adoption metrics.

The April-2026 framing above ("spec nearing 1.0, adoption growing") is now stale. The **DTCG Design Tokens Format Module reached its first stable version, 2025.10, on 2025-10-28** — a production-ready, vendor-neutral standard, not a draft. What the stable spec added:

- **Theming / multi-brand** — light/dark, accessibility variants, and brand themes without file duplication (native, not tooling-bolted-on).
- **Modern color** — full Display P3, Oklch, and all CSS Color Module 4 spaces, matching how design tools actually represent color.
- **Rich token relationships** — inheritance, aliases, and component-level references (the three-tier global→alias→component architecture this pack already recommends is now spec-blessed).
- **Cross-platform codegen** — one token file generates iOS, Android, web, and Flutter output.

**Production ecosystem (existence proofs, not endorsements)**: reference implementations in Style Dictionary, Tokens Studio, and Terrazzo; 10+ tools support or are implementing it (Penpot, Figma, Sketch, Framer, Knapsack, Supernova, zeroheight). Editors/contributors spanned Adobe, Amazon, Google, Microsoft, Meta, Salesforce, Shopify, Figma, and others.

**Why this matters for AI-agent readiness**: the four-precondition table earlier in this pack names "Tokens published as DTCG JSON as the source of truth" as the token-exposure precondition. With DTCG now stable rather than moving, that precondition is a commitment to a settled foundation, not a bet on a spec that might shift mid-adoption. The downstream skills that gate on this precondition — `/figma-agent-brief` (refuses below Score 4 on token exposure) and `/generative-ui-spec` — can now treat DTCG-source-of-truth as durable. **Recommendation stands and strengthens: pick DTCG JSON as the single source of truth and generate CSS vars, TypeScript types, and Figma Variables from it.**
...
- `ai-agent-readiness-rubric.md` — the standalone reusable rubric pack extracted from the audit.
