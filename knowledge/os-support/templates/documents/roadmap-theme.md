# Roadmap Theme: [THEME NAME]

**Theme ID**: RT-YYYY-NNN
**Owner**: [Name]
**Timeframe**: [Quarters]
**Investment**: [% capacity]
**Vision to Value Phase**: Phase 3 (Strategic Commitments)

## Theme Overview

**Description**: [What this theme is about]

**Strategic Rationale**: [Why it matters]

**Customer Value**: [What customers get]

## Strategic Alignment

**Supports Bet**: [Link to strategic bet]
**Company Goal**: [Which goal]

## Vision to Value Phase Status

| Phase | Status | Key Artifacts |
|-------|--------|---------------|
| 1 - Strategic Foundation | ✓ Complete | Market analysis, vision |
| 2 - Strategic Decisions | ✓ Complete | Business case, pricing |
| 3 - Strategic Commitments | In Progress | This roadmap theme |
| 4 - Coordinated Execution | Pending | Launch plan |
| 5 - Business Outcomes | Pending | Success metrics |

## Success Metrics

| Metric | Baseline | Target | Timeframe |
|--------|----------|--------|-----------|
| | | | |

## Initiatives

### Initiative 1: [Name]
- **Priority**: P0 / P1 / P2
- **Effort**: S / M / L / XL
- **Owner**: [Name]
- **Target**: [Quarter]

### Initiative 2: [Name]
[Same structure]

## Dependencies

| Dependency | Type | Owner | Status |
|------------|------|-------|--------|
| | | | |

## Risks

| Risk | Probability | Impact | Mitigation |
|------|-------------|--------|------------|
| | | | |

---

*Template aligned with v2.2.0 - includes Vision to Value Phase tracking*
