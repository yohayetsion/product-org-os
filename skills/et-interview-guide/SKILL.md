...
```
/interview-guide --jd the supplied job-description input --jurisdiction US-DE --panel-roles "hiring-manager,peer,cross-functional,hr"
...
```
/interview-guide update the prior guide input --jurisdiction US-NYC
```
...
|---|---|---|
| JD file | Yes | `the supplied job-description input` (ideally output of `/job-description-generator`) |
...
| Prior guide (Update/Check) | Yes in Update/Check mode | `the prior guide input` |
...
Scope of a standard run: JD-to-competency derivation + behavioral question generation with rubrics + panel assignment + forbidden-question register + proxy-laden rapport scan + calibration protocol.
...
```
User: /interview-guide --jd the supplied job-description input --jurisdiction US-DE --panel-roles "hiring-manager,peer,cross-functional,hr"

/interview-guide v1.0.0 — loading:
  - JD: the supplied job-description input (6 essential functions; source: /job-description-generator output)
