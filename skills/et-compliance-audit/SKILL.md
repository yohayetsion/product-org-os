...
```
/compliance-audit «AI Platform» --framework soc2
/compliance-audit "<vendor> <customer> deployment" --framework <framework-id>
/compliance-audit «DTC Brand» --framework gdpr
...
```
/compliance-audit update <path>/compliance-audit-<framework>-<YYYY-MM-DD>.md
```
...
```
/compliance-audit "<vendor> <customer> deployment" --framework <framework-id> --mode adversarial --tiebreaker "<named human>"
```
...
| Organization or initiative name | Yes | "<vendor> <customer> deployment" |
| Scope statement | Yes | "<product category> deployed at a regulated customer. State the workforce size, whether inference is cloud or on-premise, and any data-residency requirement" |
...
| Jurisdictions of operation | Yes (may be multi) | "<primary jurisdiction>; note any cross-border residue and the parent entity's domicile" |
...
| Stakeholder list | If known | "<name> (vendor CTO), <name> (product lead), <name> (customer sponsor)" |
...
| Tiebreaker (adversarial mode only) | Yes if adversarial | "<named human tiebreaker>" |
...
- **Hybrid frameworks** (e.g. a `<sector>-<topic>` bundle combining a national privacy statute, a sector regulator's directive and local labour law): the user defines the bundle in the input, listing which underlying frameworks are in scope. The skill treats the bundle as a custom framework and runs the audit against the bundled scope.
...
### a colocated current-status sidecar sidecar pattern for date-sensitive regulatory content
...
```
<path>/
  compliance-audit-<framework>-<YYYY-MM-DD>.md          ← audit output
  compliance-audit-<framework>-<YYYY-MM-DD>-status.md   ← sidecar
```
...
>
> **Jurisdictions in Scope:** {e.g., "the customer's home jurisdiction + U.S. Delaware residue"}. Findings are framed against the named framework's control requirements. If your operations extend to additional jurisdictions or additional regulated activities, treat every finding below as a hypothesis to re-verify per regime.

### 2. Audit Metadata Block

A small labeled block under the disclaimer with:

- **Framework**: the `--framework` value (e.g., `<framework-id>`)
...
- **Sidecar Status File**: path to the a colocated status sidecar sidecar, if one exists
...
- **What the org has in place**: grounded in the source documents with specific citation (e.g., "<customer> regulatory compliance plan Section 2.3"). If nothing is in place, say `[ABSENT]` and name where it would normally live.
...
- CORRECT: "Control gap vs a national banking outsourcing directive §4.2 (Material Outsourcing Classification): the source documents do not indicate whether «Bank»'s compliance team has classified «Security Vendor» as a material outsourcing arrangement. Suggest confirming the classification with the «Bank» compliance contact before the technical deep-dive. If material, heightened requirements apply (pre-notification to regulator, enhanced oversight, exit plan). Engage `compliance-officer` for the classification process and `general-counsel` if the classification is contested."
- INCORRECT: "The org is non-compliant with a national banking outsourcing directive because the material outsourcing classification has not been completed."
...
- The jurisdictions in scope (including secondary jurisdictions with residue exposure — e.g., a Delaware parent of a non-US operating sub has residue Delaware exposure even if the operating activity is elsewhere)
...
Actions must be specific and actionable. "Improve access control" is not a roadmap action. "Document the quarterly access review process for the <system> connector (<directive> §5.3 evidence requirement); owner <name>; effort S; depends on the architecture diagram (pending)" is a roadmap action.
...
No ROI line is emitted — the skill reports nothing about its own value. Hours are computed
downstream from the owner-affirmed activity table by the telemetry pipeline; the manual
baseline recorded in this section is a time-basis input to that table, not a figure the skill
reports. Wherever those computed hours are displayed, the framing stays **drafting and triage**
per `roi-display.md` — never "review."
...
**Sidecar pattern dependency**: the a colocated current-status sidecar sidecar file is authored alongside each audit output when date-sensitive regulatory content is referenced. The sidecar has no separate knowledge pack — it is a per-audit artifact stored next to the audit output file.
...
```
User: /compliance-audit "<vendor> <customer> deployment" --framework <framework-id>

/compliance-audit v1.0.0 - reading:
  - <path>/<customer> regulatory compliance plan
  - <path>/<customer>-master-plan.md
  - <path>/risk-analysis-<customer>-deal.md (prior A5)
  - <path>/contract-review-<agreement>.md (prior A1)
  - <path>/<vendor>-enterprise-license-agreement.pdf (schedules)

Framework adapter: inline bundle definition
  Bundled regimes:
    - the customer's national data-protection statute, and any amendment in force
    - its data-security regulations
    - the sector regulator's cyber-defence directive
    - collective-agreement or works-council consultation law, where it applies
    - cross-border transfer restrictions, including any sector secrecy rule
    - labour-court precedent on workplace monitoring (proportionality)

Collecting inputs:
  - Organization: <vendor> deploying at <customer>
  - Scope: <product category> at a regulated customer of <N> employees;
    cloud or on-prem TBD; data-residency requirement TBD
  - Jurisdictions in scope: <primary>; <parent entity domicile> residue
  - Stage: Pre-deployment readiness, pre-signature
  - Source documents: 5 files enumerated above
  - Stakeholders: <name> (vendor CTO), <name> (product lead),
    <name> (customer sponsor), TBD customer compliance contact

Producing output at:
  <path>/compliance-audit-<framework>-<YYYY-MM-DD>.md
  <path>/compliance-audit-<framework>-<YYYY-MM-DD>-status.md (sidecar)
