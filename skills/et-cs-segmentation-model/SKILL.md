...
Every new skill must be birth-tested against a real or synthetic customer portfolio before it is declared v1.0.0 ready. For `/cs-segmentation-model`, the birth test was run against the **«AI Platform» target customer profile** (builder-consumer split, $10/mo individual workspace + $25/mo all-teams bundle) as it exercises all four tiers cleanly — the «Security Vendor» portfolio is enterprise-only and does not test the PLG or Tech-Touch tiers.

The birth test output lives at: `the original private validation artifact`

The birth test validates:
- Axis definition (log-scale ARR bands work for «AI Platform»'s long-tail distribution)
...
Time saved on drafting and triage: the skill produces in ~20 minutes what a CS Ops lead would typically draft in 6–10 hours across a working session — axis definition, rubric weights, customer placement, capacity math, and quality gates.
