...
| Input | Purpose |
|---|---|
| `--readiness-score FILE` | Path to the design-system AI-readiness audit (output of `the supplied design-system AI-readiness audit`). Enables the precondition check. |
...
**Required minimum**: ≥3/5 on **typed props** AND ≥3/5 on **variant coverage** (from `the supplied design-system AI-readiness audit`).

**If `--readiness-score` is provided**:
- Read the audit file. Extract typed-props score and variant-coverage score.
- If either <3/5: output a blocking warning, refuse to produce the brief, and point the user to `the supplied design-system AI-readiness audit` to improve the library first.
- If both ≥3/5: proceed.

**If `--readiness-score` is NOT provided**:
- Prompt the PM: "This skill requires a design system AI-readiness score of ≥3/5 on typed props and variant coverage. Have you assessed the library against those two dimensions? [yes / no / don't know]"
...
|---|---|---|
| Input | `the supplied design-system AI-readiness audit` | Provides the readiness score this skill requires |
...
This skill's first real output is a brief for the **«Security Vendor» risk-dashboard alert detail card**, saved at `the original private validation artifact`. The birth test exercises:
...
Time saved on **drafting and triage** of a Figma AI brief (not on design work itself). A thoughtful Figma AI brief takes a PM ~45-90 minutes to draft well from a blank page; this skill compresses that to ~15 minutes of structured fill-in, for ~30-60 minutes saved per brief.
