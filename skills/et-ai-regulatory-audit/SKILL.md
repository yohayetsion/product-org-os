...
### Step 2 — Load `knowledge/shared/compliance-frameworks.md` sidecar; check freshness

Read `knowledge/shared/compliance-frameworks.md`. Confirm:
...
| **CCPA / CPRA (California)** | California consumer data processing. ADMT rules (automated decision-making) apply to AI systems making automated decisions about California consumers once finalized — see `knowledge/shared/compliance-frameworks.md` for current ADMT rulemaking status. |
...
| **Israeli Protection of Privacy Law (PPL) 5741-1981** | Personal data processing touching Israeli residents. 2025 amendment expanded obligations; check `knowledge/shared/compliance-frameworks.md` for phased effective dates. |
| **Bank of Israel Directive 361** | Israeli banking sector. Sector-specific, banking-only. Pointer to `/compliance-audit --framework financial-services-privacy` for the deep dive. |
...
- Sidecar freshness: {embedded current-status authority last_verified date; staleness flag if >90 days}
...
- [ ] `knowledge/shared/compliance-frameworks.md` sidecar checked for freshness and [CHECK]/[VERIFY] flags
...
- **Input**: `compliance-frameworks` knowledge pack v1.1.0 + `knowledge/shared/compliance-frameworks.md` sidecar
...
2. **`knowledge/shared/compliance-frameworks.md` sidecar read at runtime.** No date-sensitive claim in the output is hardcoded; every version number, effective date, and status claim traces to a sidecar entry. Staleness >90 days is flagged at the top of the output.
...
- **Primary pack**: `compliance-frameworks` v1.1.0 at `knowledge/shared/compliance-frameworks.md`
- **Sidecar**: `knowledge/shared/compliance-frameworks.md` (read at runtime, Step 2)
- **Sources manifest**: `knowledge/shared/compliance-frameworks.md` (canonical URLs for mechanical re-verification)
...
- **v1.0.0** (2026-04-11) — Initial authoring. Phase 5A C1.2b. First real consumer of the `knowledge/shared/compliance-frameworks.md` sidecar pattern. Consumes `compliance-frameworks` v1.1.0 Section 3 mapping table top-down. Four-sided boundary against `/compliance-audit`, `/ai-control-audit`, `/privacy-policy-audit`, `/risk-analysis`. Birth test: «AI Platform» organizational regulatory posture.
