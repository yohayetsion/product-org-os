...
```
/nda-triage "«DTC Brand» vendor NDA from Acme Corp"
/nda-triage examples/nda-draft-mutual-2026.pdf
/nda-triage "«AI Platform» inbound NDA from enterprise prospect"
```
...
```
/nda-triage update nda-triage-<counterparty>-<date>.md
```
...
```
/nda-triage nda-mna-target --mode adversarial --tiebreaker "«Reviewer»"
```
...
- **Clause library version used**: date of `the operator-supplied clause library` at review time
...
- **Birth Test — DEFERRED** — Per «Reviewer»'s Option E decision on 2026-04-11, A2 ships as v1.0.0 **without a birth test**. See the Attribution and Maintenance section for the full rationale.
...
**Clause library dependency**: `/nda-triage` reads primarily from the Confidentiality category of `the operator-supplied clause library` (bootstrapped by A1's birth test against the seed agreement). Secondary references: Jurisdiction & Governing Law, Term/Termination, IP, and Data Protection categories. When the library is updated, `/nda-triage` picks up new patterns automatically. The library's version (date) is recorded in the NDA Metadata block of every output for traceability.

### Birth Test — DEFERRED per Option E

**Status at v1.0.0**: No birth test. Shipping anyway.

**Rationale**: The Phase 3 Sub-phase 3.1 plan named a completed «DTC Brand» vendor NDA as the birth-test target for A2. «Reviewer» searched the workspace for a completed «DTC Brand» vendor NDA on 2026-04-11 and none exists as a file. Rather than fabricate a synthetic NDA or a fake birth-test output (which would violate first-principles authoring — no fabricated test fixtures), «Reviewer» chose **Option E**: ship A2 without a birth test, explicitly flagged as "awaiting first real triage," and let the first real NDA that arrives through the «Holding Co» pipeline («DTC Brand» corporate gifting, «Design Agency» vendor work, ProductBeacon practice engagements, «Security Vendor» enterprise sales, or any other channel) become the natural v1.0.0 birth test.

**When the first real NDA arrives**: Run `/nda-triage` on it, capture the verdict and the 3-5 driving findings, validate the decision-tree branches against the actual text, and feed the observed clause patterns back into the Confidentiality category of `the operator-supplied clause library`. The resulting triage output becomes the v1.0.0 birth test and gets noted in the Changelog as validating (or requiring correction to) the decision tree.
...
Producing output at:
  nda-triage-<counterparty>-<date>.md
