...
argument-hint: '[product name] — e.g., "«Security Vendor»", "«AI Platform»"'
user-invocable: true
metadata:
...
Create `{Product}/the product workspace's vision-to-value document map` with:
...
| `favicon` | From the product's live website |
| `logo` | From the product's live website |
...
|------|----------|---------|
| Markdown registry | `{Product}/the product workspace's vision-to-value document map` | Source of truth, text-searchable |
