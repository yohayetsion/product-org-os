...
Every new skill must be birth-tested against a real or synthetic portfolio before v1.0.0. For `/health-score-design`, the birth test runs against the **«AI Platform»** context — consistent with the `/cs-segmentation-model` birth test that preceded it, so the two skills chain cleanly.

«AI Platform» is a Tier 3 case by construction: a pre-PMF product with no churn history and a mandatory 2-quarter recalibration gate. It exercises:
...
- Cold-start variant («AI Platform» will have many <90-day customers in year 1)
...
Time saved on drafting and triage: the skill produces in ~25 minutes what a CS Ops lead + CSM working session would typically draft in 8–14 hours — signal catalog, per-segment weight matrices, cold-start variant design, calibration tier decision, recalibration trigger specification.
