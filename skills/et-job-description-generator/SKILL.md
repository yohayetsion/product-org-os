...
```
/job-description-generator update the supplied prior job-description artifact --jurisdiction US-NYC
```
...
| Prior JD (for Update/Check) | Yes in Update/Check mode | `the supplied prior job-description artifact` |
...
Scope of a standard run: a first-pass bias scan + structural draft + comp alignment check.
...
```
User: /job-description-generator --role "Customer Success Lead" --jurisdiction US-DE --comp-band "$130-170k base + equity" --essential-functions «ai-platform»/cs-lead-essential.md
