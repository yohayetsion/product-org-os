...
- **Counterfactual harness**: `hr-ai-governance` Section 5 specifies the 200-fixture counterfactual test. The fixture files live at `the operator-supplied counterfactual fixture set` (scheduled engineering task per Section 5.1). This skill **references** the harness; it does not author or own the harness. Harness runs: skill ship, every 180 days, every model change, every prompt revision.
...
Batch-shape variants (time-basis inputs to the activity table, not multipliers):
- **Simple** (homogeneous resume format, same jurisdiction, no GINA/union flags): ~2.1 hrs
- **Standard** (mixed format, single jurisdiction, occasional flags): ~4.2 hrs
- **Complex** (multi-jurisdiction batch, heterogeneous format, multiple flags, counterfactual re-verification): ~6.3 hrs
...
```
User: /resume-summarizer --batch ./candidate-batch-2026-Q2/ --jurisdiction US-NYC --role "Senior Backend Engineer" --deployer-attestation ./attestations/«ai-platform»-2026.json
...
/resume-summarizer v1.0.0 — loading:
  ...
  - Deployer attestation: «ai-platform»-2026.json
