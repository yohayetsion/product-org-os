---
name: context-recall
description: 'Query past decisions, bets, learnings, and documents by topic from the context registry. Activate when: "what did we decide about", "find past decisions", "recall context on", historical context,
  prior decisions Do NOT activate for: saving new context (/context-save), recalling feedback (/feedback-recall), recalling past interactions (/interaction-recall)'
...
- `/context-recall pricing product:«Security Vendor»` → Search filtered to «Security Vendor» product
...
```
User: /context-recall roadmap product:«Security Vendor»

Agent: Searches all context indexes filtered to «Security Vendor» product only.
       Returns «Security Vendor»-specific roadmap documents, decisions, and bets.
```
