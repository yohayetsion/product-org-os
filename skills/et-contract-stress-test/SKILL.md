...
- the seed agreement before first enterprise signature
- M&A term sheet before the LOI lock date
- IP licensing agreement where «Security Vendor» is licensing a Frontier Model from a third party
...
```
/contract-stress-test enterprise-licence-agreement.pdf --tiebreaker "«Reviewer»"
...
```
/contract-stress-test update examples/contract-stress-test-eula-v2-2026-04-11.md
```
...
|---|---|---|
| Contract document (path or content) | Yes | `examples/Collaterals/enterprise-licence-agreement.pdf` |
| **Named human tiebreaker** | **Yes — skill refuses output without it** | "«Reviewer»" / "`general-counsel`" / "«CTO»" |
| Counterparty context | Yes | "top-tier bank in a strict-data-residency jurisdiction, regulated by its national banking authority" |
...
| Prior review output | Optional but recommended | `the supplied prior-review artifact` |
| Jurisdiction | Yes | "the customer's home jurisdiction + U.S. Delaware (multi-jurisdictional EULA)" |

**Named tiebreaker is NON-NEGOTIABLE.** If no tiebreaker is named, the skill asks the user rather than producing output. This is enforced because Pattern 5's stop criteria include "hand off contested findings to named human" — without a named human, contested findings have nowhere to go and the pattern deadlocks. Default tiebreaker for legal deliverables in this workspace: **«Reviewer»**.
...
- Access to: the contract document, the clause library (`the operator-supplied clause library`), the prior review output if provided, and the stake assessment
...
Consultations are attributed in the finding: "I consulted 🔒 Privacy Counsel, who confirmed that the DPA's cross-border transfer carve-out is stronger than GDPR requires but weaker than a national banking cyber-defense directive expects for outsourced data processors."
...
Adversarial review is a more specialized motion than a standard triage: it requires fresh-context discipline and a structured honesty mechanism. Senior practitioners who can maintain fresh-context role separation alone (without tooling) are rare.
...
```
User: /contract-stress-test licence-agreement-revision-comparison.docx --tiebreaker "«Reviewer»"

/contract-stress-test v1.0.0 — reading:
  - examples/Collaterals/licence-agreement-revision-comparison.docx (12-row comparison table)
  - examples/Collaterals/enterprise-licence-agreement.pdf (near-final draft)
  - the supplied prior-review artifact (prior review, 14 findings)

Collecting inputs:
  - Contract: a licence-agreement revision diff (stress-testing the v2 as near-final draft)
  - Counterparty: First enterprise deal — «Bank» (a regulated bank)
  - Drafter: «CTO» / «Security Vendor» Legal
  - Adversarial agent: 📄 Contracts Counsel (fresh-context spawn, simulated)
  - Iteration: 1 of 2
  - Tiebreaker: «Reviewer» (confirmed before review started)
  - Stake: First enterprise deal with regulated bank; cost of missed clause is
    multi-year reputational and contractual exposure
  - Prior review: the supplied prior-review artifact (14 findings)
  - Jurisdiction: the customer's home jurisdiction + U.S. Delaware

Iteration check: 1 of 2 ✓
Fresh-context spawn: confirmed (simulated — single-agent deployment)
Clause library loaded: the operator-supplied clause library @ 2026-04-11

Producing output at:
  the original private validation artifact
...
- **1.0.0 (2026-04-11)** — Initial authoring. First-principles during Phase 3 Sub-phase 3.0. First skill to directly invoke Pattern 5 Adversarial Review from `delegation-protocol.md` as its core method. Birth-tested against a licence-agreement revision diff with A1's prior review as the prior-review reference. Closes Phase 3. Substantive review on the 72-hour subsequent-similar SLA inherited from A5/A1/A2/A3/A4.
