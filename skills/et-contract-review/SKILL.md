...
```
/contract-review enterprise-licence-agreement.pdf
/contract-review "«DTC Brand» watch vendor SOW"
/contract-review "«AI Platform» reseller agreement - draft from Acme"
```
...
```
/contract-review update examples/contract-review-eula-v2-2026-04-11.md
```
...
```
/contract-review enterprise-licence-agreement --mode adversarial --tiebreaker "«Reviewer»"
```
...
| Counterparty context | Yes | "top-tier bank in a strict-data-residency jurisdiction, regulated by its national banking authority" / "US SaaS vendor, mid-market" |
| Jurisdiction (or authorization to assume + flag) | Yes | "the customer's home jurisdiction + U.S. Delaware" |
| Known red lines | If known | "No uncapped indemnity", "No assignment without consent", "Data residency must be in-region" |
| Specific clauses of concern | If any | "Section 8.2 LoL cap" / "Schedule D Order Form" |
| Tiebreaker (adversarial mode only) | Yes if adversarial | "«Reviewer»" |

If jurisdiction is unknown, the skill reads the governing-law clause of the contract itself and reports what IT says, rather than silently picking a default. If the contract is multi-jurisdictional (as the seed agreement is — one jurisdiction for non-Americas customers, Delaware for Americas customers), the skill names both and flags which one applies to the deal under review.
...
- **Clause library version used**: the date of `the operator-supplied clause library` at the time of review
...
- The clause categories that ARE present (using `the operator-supplied clause library` as the checklist)
...
For each clause category present, the skill pulls the reviewer questions and red flags from `the operator-supplied clause library` and runs them against the actual clause text. For each:
...
Consultations are attributed in the Findings section: "I consulted 🔒 Privacy Counsel, who noted that the DPA's 2-hour breach notification window is tighter than GDPR's 72-hour baseline but matches a national banking cyber-defense directive's expectations." Ownership of the contract review stays with Contracts Counsel.
...
**Clause library dependency**: `/contract-review` reads from `the operator-supplied clause library`. When the library is updated, `/contract-review` picks up the new patterns automatically. The library's version (date) is recorded in the Contract Metadata block of every output for traceability.
...
```
User: /contract-review enterprise-licence-agreement.pdf

/contract-review v1.0.0 — reading:
  - examples/Collaterals/enterprise-licence-agreement.pdf
    (core agreement plus inline schedules)

Collecting inputs:
  - Contract: an enterprise software licence agreement
  - Parties: «Security Vendor» + [Customer]
  - Deal Type: SaaS / enterprise license
  - Our Stance: Paper on paper (we authored)
  - Counterparty Context: First use against «Bank» (a regulated bank)
  - Jurisdiction: Section 14.1 — the customer's home jurisdiction outside the Americas, Delaware within.
    «Bank» is outside the Americas → Section 14.1(a) applies.
  - Clause library version: 2026-04-11 (seed corpus; this review is part of the bootstrap)

Producing output at:
  examples/contract-review-eula-v2-2026-04-11.md
...
- **1.0.0 (2026-04-11)** — Initial authoring. First-principles during Phase 3 Sub-phase 3.0. First contract-specific skill. Birth-tested against an enterprise software licence agreement with 14 clause-level findings; bootstrapped the initial `the operator-supplied clause library` library from the review. Substantive review passed by ⚖️ General Counsel on the 72-hour subsequent-similar SLA (A5 `/risk-analysis` validated the template pattern earlier today); scaffolding review passed by 📋 Director of Legal Affairs.