...
If the entire context structure is missing, create it (see CLAUDE.md -> "Create the context layer"), then continue.
...
| **Product** | Which product (for multi-product orgs, e.g., «Security Vendor», «DTC Brand») | If applicable |
...
1. **Reserve** the feedback ID — never by checking the index for the next number:

   ```bash
   python tools/allocate-id.py --ns FB --note "<short reason>"
   ```

   Use the printed id verbatim. **If it exits non-zero, do NOT guess a number** — re-run it; it refuses exactly when it cannot prove the id is free. Read-the-index-and-add-one is what collided five ids across five namespaces on 2026-08-02: two concurrent sessions read the same maximum before either wrote. Note also that `FB-` records live in **two** places (`context/feedback/YYYY/` files *and* index rows) and today 11 files exist with no index row — so the index alone is not the full picture; the allocator reads both. (Collision-safe across concurrent sessions on one machine, not across machines. If `tools/allocate-id.py` is absent from this workspace, fall back to reading the index — and verify the id is free immediately before **and after** the write.)
