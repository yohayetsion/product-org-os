...
Birth test: `Northwind/Product/contract-stress-test-eula-v1-v2-diff-2026-04-11.md` (EULA v1→v2 diff, iteration 1)
...
- [x] **Standard disclaimer block present at top of output?** Yes — verbatim UPL block with `{jurisdiction}` filled in as "Israel (Section 14.1(a) for Meridian Bank); multi-jurisdictional with Delaware Section 14.1(b) for Americas."
...
| A5 `/risk-analysis` | Phase 3 | Northwind Meridian Bank Deal | Pattern 1 Consultation (default) |
| A1 `/contract-review` | Phase 3 | Northwind EULA v2 | Pattern 1 Consultation (default) + `--mode adversarial` |
| A2 `/nda-triage` | Phase 3 | None (Option E — decision-tree pattern) | Pattern 1 Consultation |
| A3 `/privacy-policy-audit` | Phase 3 | Atlas privacy program | Pattern 1 Consultation |
| A4 `/compliance-audit` | Phase 3 | Northwind Israeli regulatory readiness | Pattern 1 Consultation + sidecar pattern |
| **A7 `/contract-stress-test`** | **Phase 3** | **Northwind EULA v1→v2 diff** | **Pattern 5 Adversarial Review (core method)** |
